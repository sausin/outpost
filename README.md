...
<p>
  ...
  <a href="https://github.com/sausin/outpost/pkgs/container/outpost"><img src="https://img.shields.io/badge/ghcr.io-sausin%2Foutpost-2496ED?logo=docker&logoColor=white" alt="GHCR"></a>
  <a href="LICENSE"><img src="https://img.shields.io/badge/license-MIT-green.svg" alt="License: MIT"></a>
  <img src="https://img.shields.io/badge/python-3.12+-3776AB?logo=python&logoColor=white" alt="Python 3.12+">
  <img src="https://img.shields.io/badge/built%20with-FastAPI-009688?logo=fastapi&logoColor=white" alt="Built with FastAPI">
  <img src="https://img.shields.io/badge/managed%20by-uv-DE5FE9" alt="Managed by uv">
...
| Mode | What it does |
|---|---|
| **Internal only** | Sidecar pattern. Exposes `localhost:8080`. No TLS. Use when your agent runs on the same host or Docker network. |
| **Public via domain** | Caddy + Let's Encrypt auto-HTTPS on `:80`/`:443`. Provide a domain + email, that's it. |
...
```bash
docker pull ghcr.io/sausin/outpost:latest
```

Multi-arch (`linux/amd64`, `linux/arm64`). Tagged with `latest`, `v0.1`,
`v0.1.0`, etc.
...
| Footprint | 32 MB Docker | 5–50 MB | 100+ MB | varies |
...
`AuthModule` protocol in ~50 lines of Python and reference it as
`auth: {type: plugin, module: my_pkg.my_mod:MyAuth}`.
...
- [x] Multi-arch GHCR images
- [ ] Cloudflare Workers TypeScript runtime (same YAML format, edge deploy)
...
1. `uv run ruff check app/ outpost_cli/` clean
2. `uv run ruff format --check app/ outpost_cli/` clean
3. `uv run pyright app/ outpost_cli/` clean
4. New providers: include a YAML in `app/builtin_providers/` with `enabled: false`
   so users can opt in; document the auth flow in a comment block at the top.
5. New auth modules: drop in `app/auth/modules/`, register in `app/auth/registry.py`.
